...
def detect_booked_shifts(proc_image, image, image_path, year, month):
    ...
    # Remove any coloured pixels from grey mask to avoid overlap
    mask_gray = cv2.bitwise_and(mask_gray, cv2.bitwise_not(mask_coloured))
    # Combined mask for contour detection (coloured = booked, grey = unavailable)
    ...
    for cnt in contours:
        ...
        if w > 10 and h > 10:
            block_top = band_y1 + y
            # Classify: coloured pixels → booked, grey-only pixels → unavailable
